...
def ConvertToList(filename, words, sents, model, all_lens, maxlen_sentences, maxlen_words):
    ...
    overall_list = (LoadFileSingle(filename+'.json', 'overall'))
    reviewerID_list = (LoadFileSingle(filename+'.json', 'reviewerID'))
    asin_list = (LoadFileSingle(filename+'.json', 'asin'))
    data_x = []
    data_y = []
    data_y1 = []
    data_x1 = []
    ...
    for i, sent in enumerate(sents):
        ...
        labels = [0] * 6
        labels[int(overall_list[i])] = 1
        data_y1.append(labels)
        data_x.append(doc)
